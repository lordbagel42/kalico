...
def test_velocity_setpoint_tracks_constant_velocity(mock_odrive):
    # "v" is a real ASCII protocol setpoint command (transport.py's
    # send_velocity_setpoint) -- unlike "p", it must integrate position
    # at a constant rate rather than lagging towards a fixed target (see
    # AxisMotion.advance() in scripts/odrive_mock.py).
    _, port = mock_odrive
    with serial.Serial(port, 115200, timeout=2) as ser:

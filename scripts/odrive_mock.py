        ...
        props.update(
            {
                ...
                p + "encoder.shadow_count": 0,
                p + "encoder.hall_state": 1,
...
class AxisMotion:
    ...
    def advance(self, now, tau):
        ...
        if self.vel_target is not None:
            self.vel_turns = self.vel_target
            self.pos_turns += self.vel_target * dt
            return
        if tau > 0.0:
